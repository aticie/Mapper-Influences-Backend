...
@router.get(
    "/search/{query}",
    response_model=OsuSearchResponse,
    summary="search users using osu api",
)
@cache(
    namespace=OSU_API_CACHE_NAMESPACE,
...
@router.get(
    "/search_map",
    response_model=OsuSearchMapResponse,
    summary="search beatmaps using osu api",
)
@cache(
    namespace=OSU_API_CACHE_NAMESPACE,
    expire=OSU_API_SEARCH_CACHE_EXPIRE,
...
async def get_beatmap_osu_parsed(
    requester: Requester, access_token: str, beatmap_id: int
):
...
async def get_beatmapset_osu_parsed(
    requester: Requester, access_token: str, beatmapset_id: int
):
