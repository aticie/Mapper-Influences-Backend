...
async def add_influence(
    ...
    user_osu = await get_user_osu_parsed(
        requester, user["access_token"], influence.influenced_to
    )
...
@router.get(
    "/{user_id}",
    response_model=list[InfluenceDBModel],
...
async def remove_influence(
    ...
    removed_influence_user = await mongo_db.get_user_details(influenced_to)

    activity_details = ActivityDetails(
        influenced_to=ActivityUser.model_validate(removed_influence_user)
    )
    await activity_ws.collect_acitivity(
        ActivityType.REMOVE_INFLUENCE, user_data=user, details=activity_details
    )
