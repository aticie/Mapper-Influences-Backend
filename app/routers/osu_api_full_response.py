# Full osu! API response with caching
...
async def get_beatmapset(
    ...
    if type == "beatmapset" or type is None:
        return await get_beatmapset_osu(request_session, access_token, id)
    elif type == "beatmap":
        beatmap = await get_beatmap_osu(request_session, access_token, id)
        return await get_beatmapset_osu(request_session, access_token, beatmap["beatmapset_id"])
    else:
        raise HTTPException(
...
async def get_user(
    ...
    return await get_user_osu(request_session, access_token, user_id)
...
async def get_user_beatmap(
    ...
    return await get_user_beatmaps_osu(request_session, access_token, beatmap_id, type)
...
async def search(
    ...
    return await search_user_osu(request_session, access_token, query)
...
async def search_map(
    ...
    return await search_map_osu(request_session, access_token, str(request.query_params))


async def get_beatmap_osu(session: ClientSession, access_token: str, beatmap_id: int):
    beatmap_url = f"https://osu.ppy.sh/api/v2/beatmaps/{beatmap_id}"
    auth_header = {"Authorization": f"Bearer {access_token}"}
    async with session.get(beatmap_url, headers=auth_header) as response:
        return await response.json()


async def get_beatmapset_osu(session: ClientSession, access_token: str, beatmapset_id: int):
    beatmapset_url = f"https://osu.ppy.sh/api/v2/beatmapsets/{beatmapset_id}"
    auth_header = {"Authorization": f"Bearer {access_token}"}
    async with session.get(beatmapset_url, headers=auth_header) as response:
        return await response.json()
...
async def get_user_beatmaps_osu(session: ClientSession, access_token: str, user_id: int, type: str):
    user_maps_url = f"""https://osu.ppy.sh/api/v2/users
                      /{user_id}/beatmapsets/{type}"""
    auth_header = {"Authorization": f"Bearer {access_token}"}
    async with session.get(user_maps_url, headers=auth_header) as response:
        return await response.json()


async def search_user_osu(session: ClientSession, access_token: str, query: str):
...
async def search_map_osu(session: ClientSession, access_token: str, query: str):
