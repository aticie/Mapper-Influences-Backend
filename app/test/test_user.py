...
@pytest.mark.asyncio
async def test_add_beatmap(test_client, headers, mongo_db, test_user_id):
...
@pytest.mark.asyncio
async def test_remove_beatmap(test_client, headers, mongo_db, test_user_id):
...
async def test_influence_order(test_client, headers, mongo_db, test_user_id):
    ...
    assert response.status_code == 200
    response = await test_client.get("influence/873961", headers=headers)
